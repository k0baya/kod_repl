  ...
  Paint:
    function(opt) {
      ...
      // copy paint object
		...
		// add by warlee; 渐变颜色,引用其他地方的情况处理;
		var index = 0;
		while(this.linearGradient && $(this.linearGradient).attr('xlink:href') && index < 10){
			this.linearGradient = $($(this.linearGradient).attr('xlink:href')).get(0).cloneNode(true);index++;
		}
    }
};
...
//      if (j == 0) selectedBBoxes[0] = svgedit.utilities.getBBox(elem);
  ...
  // changed by warlee; 避免死循环; 加入长度判断 selectedElements.length > 1;
  while(selectedElements.length > 1 && selectedElements[0] == null) selectedElements.shift(0);
};
  ...
  elems.each(function() {
    ...
    if($(grad).attr('gradientUnits') === 'userSpaceOnUse') {
      ...
      if(grad.tagName === 'linearGradient') {
		...
		// add by warlee; 保留颜色渐变特性(兼容高度或宽度为0 导致x,y为NaN的问题);
...
(function() {
  ...
  canvas.setBlur = function(val, complete) {
    ...
    var elem = selectedElements[0];
	if(!elem)  return; // add by warlee; 没有选中时调整报错处理
    var elem_id = elem.id;
...
this.moveSelectedElements = function(dx, dy, undoable) {
  ...
  var undoable = typeof(undoable) == 'undefined' ? true : undoable;//changed by warlee; 默认值处理;
...
MD.Editor = function(){
  ...
  function elementChanged(window,elems){
    const mode = svgCanvas.getMode();
	if(!elems[0]) return ;// add by warlee;
  ...
  //changed by warlee 参数错误; changeBlur(ctl, completed) =>changeBlur(attr,ctl,completed)
  function changeBlur(attr,ctl, completed){
...
MD.Text = function(){
  ...
  $('#font_family_dropdown').change(function() {
    ...
    if (!isSystemFont) fam = `${fam}`; // changed by warlee; `'${fam}'` => `${fam}`
...
MD.Panel = function(){
	...
	// changed by warlee; 避免重复调用; .align_buttons => #multiselected_panel
    $('#multiselected_panel .align_button').on("click", function(){
      ...
      if (elem !== null) {
       ...
       if(panels[el_name]) {
         ...
         if(el_name === 'text') {
		   ...
		   var displayFont = (fonts[font_family] && fonts[font_family].name) || cleanFontFamily;// add by warlee;
           $('#preview_font').text(displayFont).css('font-family', cleanFontFamily === "default" ? "sans-serif" : cleanFontFamily);
           const textPath = elem.querySelector("textPath");
  ...
  var colorPicker = function(elem) {
    ...
    $("#color_picker")
      ...
      function(p) {
        ...
        // editor.paintBox[picker].setPaint(paint, true); //changed by warlee; 避免添加历史记录失败的问题处理;
...
MD.Palette = function(){
  ...
  $('#palette').on("mousemove mousedown touchstart touchmove", ".palette_item", function(evt){
    ...
    var paint = null;
    var noUndo = true;noUndo = false;//add by warlee; 允许颜色设置撤销;
...
function populateFonts(fonts){
	return window.svgLoadFonts(fonts); // add by warlee 字体处理;
  let options = `
...
// globals
$BODY.trigger('pageLoad');//add by warlee;
